...
// ワードウルフ用の市民とお題ウルフのペア
export interface WordWolfPair {
  majority: string;
  minority: string;
}

export const WORD_WOLF_PAIRS: WordWolfPair[] = [
...
export function getRandomTopic(): string {
  const index = Math.floor(Math.random() * TALK_TOPICS.length);
  return TALK_TOPICS[index];
}

export function getRandomWordWolfPair(): WordWolfPair {
  const index = Math.floor(Math.random() * WORD_WOLF_PAIRS.length);
  const pair = WORD_WOLF_PAIRS[index];
