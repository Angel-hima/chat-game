import { Room, Player, PublicRoomSummary, GameMode, WordWolfState } from './types';
import { getRandomTopic, getRandomWordWolfPair } from './topics';
...
export class RoomManager {
  ...
  // ルーム作成
  public createRoom(
    hostPlayer: Omit<Player, 'isHost' | 'isReady' | 'isOnline'>,
    name: string,
  ...
  // プレイヤー名・アバターの変更（入室中）
  public updatePlayerProfile(socketId: string, name: string, avatar: string): { room?: Room; oldName?: string } {
  ...
  public joinRoom(
    roomId: string,
    player: Omit<Player, 'isHost' | 'isReady' | 'isOnline'>,
    passcode?: string
  ...
  // プレイヤー退出
  public leaveRoom(socketId: string): { room?: Room; removedRoomId?: string } {
  ...
  // 準備完了トグル
  public toggleReady(socketId: string): Room | undefined {
    const room = this.getRoomBySocketId(socketId);
  ...
  // ゲームモード変更
  public changeGameMode(roomId: string, mode: GameMode): Room | undefined {
    const room = this.rooms.get(roomId);
    if (!room || room.status !== 'lobby') return undefined;
    room.gameMode = mode;
    return room;
  ...
  // ゲーム開始
  public startGame(roomId: string, onTick?: (room: Room) => void, onTimeUp?: (room: Room) => void): Room | undefined {
    const room = this.rooms.get(roomId);
    ...
    if (room.gameMode === 'talk') {
      ...
      const pair = getRandomWordWolfPair();
      ...
      const roles: { [playerId: string]: { word: string; isWolf: boolean } } = {};
  ...
  // お題の引き直し（雑談モード用）
  public nextTopic(roomId: string): Room | undefined {
    const room = this.rooms.get(roomId);
    if (!room) return undefined;
    room.currentTopic = getRandomTopic();
    return room;
  ...
  // タイマーの開始
  private startRoomTimer(room: Room, onTick?: (room: Room) => void, onTimeUp?: (room: Room) => void) {
  ...
  // ワードウルフの投票
  public submitVote(roomId: string, voterId: string, targetId: string, onUpdate?: (room: Room) => void): Room | undefined {
    const room = this.rooms.get(roomId);
    if (!room || !room.wordWolfState || room.wordWolfState.phase !== 'voting') return undefined;
  ...
  // ワードウルフ結果集計
  private finalizeWordWolf(room: Room) {
    if (!room.wordWolfState) return;
  ...
  // 管理者用: 部屋の強制解散
  public forceCloseRoom(roomId: string): Room | undefined {
    const room = this.rooms.get(roomId);
    if (!room) return undefined;
